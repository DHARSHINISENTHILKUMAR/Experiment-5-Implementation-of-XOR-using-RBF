...
Then, to find w used the equation Aw= Y in matrix form. Each row of A (shape: (4, 2)) consists of

### Step3:
index[0]: similarity of point with peak1 index[1]: similarity of point with peak2 index[2]:
Bias input (1) Y: Output associated with the input (shape: (4, )) W is calculated using the same equation we use to solve linear regression using a closed solution (normal equation).

### Step4:
This part is the same as using a neural network architecture of 2-2-1, 2 node input (x1, x2) (input layer) 
2 node (each for one peak) (hidden layer) 1 node output (output layer)

### Step5:
To find the weights for the edges to the 1-output unit. Weights associated would be:

### Step6:
edge joining 1st node (peak1 output) to the output node edge joining 2nd node (peak2 output) to the output node bias edge
```
## PROGRAM:
import numpy as np
...
## OUTPUT :
    ![image](https://user-images.githubusercontent.com/113699377/201341869-c0da6c5a-406c-4638-9f29-f77b695ae6d8.png)
    ![image](https://user-images.githubusercontent.com/113699377/201341992-17bf4b02-3d64-428b-b0d3-22259dbf4cea.png)
